...
export default function BidStatusPage() {
  ...
  <div className="mb-6">
    ...
    {["Entry Level", "Intermediate & Professionals", "Expert & High Level Exp."].map((label, idx) => (
      <label key={idx} className="flex items-center space-x-2 text-sm mb-3 cursor-pointer">
        <input
          type="checkbox"
          defaultChecked={label === "Entry Level"}
          className="accent-[#27548a] w-4 h-4"
        />
        <span className={label === "Entry Level" ? "text-[#27548a] font-medium" : "text-gray-700"}>
          {label}
        </span>
      </label>
    ))}
  ...
  <div className="mb-6">
    ...
    {["Hourly base", "Monthly base", "Contract base"].map((label, idx) => (
      <label key={idx} className="flex items-center space-x-2 text-sm mb-3 cursor-pointer">
        <input
          type="radio"
          name="jobType"
          ...
          className="accent-[#27548a] w-4 h-4"
        />
        <span className={label === "Monthly base" ? "text-[#27548a] font-medium" : "text-gray-700"}>
          {label}
        </span>
      </label>
    ))}
  ...
  <div>
    ...
    {["Less Than $100", "$100 To $500", "$500 To $1k", "$1k To $5k"].map((label, idx) => (
      <label key={idx} className="flex items-center space-x-2 text-sm mb-3 cursor-pointer">
        <input
          type="checkbox"
          ...
          className="accent-[#27548a] w-4 h-4"
        />
        <span className={label === "$500 To $1k" ? "text-[#27548a] font-medium" : "text-gray-700"}>
          {label}
        </span>
      </label>
    ))}
